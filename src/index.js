...
// The 2nd param to the `Safe` constructor needs to be an instance of `Key` you need
// to store it privately as well. Add a function to the Safe prototype called `unlock`
// that accepts a key. If the key matches the key that was used to create the Safe;
...
Validator.prototype.email = function(str) {
  var atAt = str.indexOf('@');
  return atAt && (0 < atAt && atAt < str.length-1);
}

if (typeof Validator === "function") {
  var valid = new Validator()
  console.assert(valid.email("name@theironyard.com"))
  console.assert(valid.email("jim@localhost"))
  console.assert(valid.email("jim@127.0.0.1"))
  console.assert(!valid.email("@theironyard.com"))
  console.assert(!valid.email("name@"))
